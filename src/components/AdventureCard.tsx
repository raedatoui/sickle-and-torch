...
const AdventureCard: React.FC<AdventureCardProps> = ({ title, description, imageSrc, reversed = false }) => {
    return (
        <div
            className={`flex flex-col ${reversed ? 'md:flex-row-reverse' : 'md:flex-row'} items-center gap-8 md:gap-12 w-full max-w-6xl mx-auto py-12`}
        >
            {/* Image Container */}
            ...
            <div className="w-full md:w-1/2 space-y-6 px-4 md:px-0 text-center md:text-left">
                <h3 className="text-2xl md:text-4xl font-bold uppercase leading-none gold-gradient-text drop-shadow-md">{title}</h3>
